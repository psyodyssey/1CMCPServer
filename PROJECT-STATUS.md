...
deliverables. Phase 1–6 закрыты ранее; пятнадцать
post-phase parallel track'ов (A, B, C, D, E, F, G,
H, I, J, K, L, M, N, O) полностью закрыты
последовательно. Последний закрытый трек —
**Parallel Track O — Dev-Time Editable Install
and Workspace Discovery** (closure commit
...
post-phase parallel track; **Parallel Track Q —
Windows Installer Path and setup.exe Delivery**
открыт как семнадцатый post-phase parallel
track. Track P — disciplined
...
parallel track. **Track P деталь — см. секцию
...
distribution-boundary.md §7` non-goals ("no GUI
installer"); `pyproject.toml` Track M comment
block (lines 44–50) — "no GUI installer", "no
Chocolatey / Homebrew / apt / conda-forge /
NuGet". Track Q — dedicated narrow track,
который эту deferral закрывает: один Windows
`setup.exe`, ordinary-user install experience
без preinstalled Python / pip / Git, uninstall
через стандартный Windows Add/Remove Programs
surface (regression под
`HKCU\Software\Microsoft\Windows\CurrentVersion\
Uninstall\<GUID>`). **Центральная честная
константа Track Q** (плана §4): платформа —
...
distribution, ~10 MB, pulled at build time;
**не** committed as binary в git). Expected
installed footprint — около 10–15 MB как
структурная стоимость. Default installer
technology — Inno Setup (directional baseline,
...
`closed` (для всего Parallel Track N —
Observability and Diagnostics Boundary — Steps 1–6
закрыты последовательно; **Q6 = NO-BUMP**
(Track N закрыт под existing `0.5.2`, без further
bump); `pyproject.toml` `version` preserved byte-
identical через все шесть Track N шагов; никаких
изменений production code ни в одном из шести
Track N шагов; никаких изменений `scripts/*`;
recipe documents existing signals без их
изменения; registry invariant `read=15 / write=25
...
meaningful commit'ов в `main`: `efb4e5c` Step 1
(planning, два planning doc'а — plan + step-map —
с Q1–Q7 directional defaults) / `d4183ca` Step 2
(descriptive baseline audit с file:line citations
across 17 existing diagnostic surfaces; key
finding: gap is integration-and-naming, not
signal-generation) / `0ff3c2b` Step 3 (normative
contract pinning **PATH A docs-only** (PATH B
selfcheck --json / PATH C log-shape contract slice
/ PATH D diagnostic-bundle helper все explicitly
rejected с grounded justification); canonical
recipe path `docs/operators/observability.md`
locked; six mandatory content elements C2–C7
locked; seven mandatory cross-references C8
locked; cross-OS posture C9 locked; nine mandatory
denial phrases locked (§9.2); exhaustive forbidden-
files surface locked (§8.5); V1–V12 + P1–P4
verification protocol locked; Q6 framed NO-BUMP
default под PATH A mirroring Track J/K/L
precedent) / `76c47ba` Step 4 (narrow PATH A
implementation: ровно один новый файл —
[`docs/operators/observability.md`](docs/operators/observability.md)
1043 lines — operator-facing observability/
diagnostics recipe с **7 first-class signals
FC1–FC7** (stderr via Python `logging` at
`--log-level INFO`; three-bucket exit codes 0/1/2;
HTTP response envelope incl. auth-failure 401 +
`WWW-Authenticate: Bearer realm="mcp"` + JSON-RPC
`-32001` failure-equivalence; `scripts/dev/selfcheck.py`
11-key=value output; `scripts/release/verify-release.ps1`
8-check release gate; install fast-path findings;
`journalctl -u <UNIT>.service` on Linux/systemd
hosts), **4 recommended-only signals R1–R4**, **10-
item out-of-scope list**, log-level-to-event
mapping inheriting Track L §9.3 verbatim, exit-
code-meaning table с file:line citations, HTTP
response envelope summary, `/healthz` non-shipping
carry-forward quoting Track J §6 verbatim, triage
recipe с тремя mandatory canonical failure modes
T1/T2/T3 + двумя optional T4/T5, cross-OS posture
Linux/systemd primary + Windows NSSM + macOS
launchd + non-supervised execution prose-only,
authoritative non-goals 9-sub-section aggregation,
operator-side verification 7 steps, 9 mandatory
explicit denials of forbidden maturity claims, 7
mandatory cross-references; ноль production code
change) / `0d3627c` Step 5 (operator docs and
observability alignment: три CLASS-1/2 modified
files — README.md + PROJECT-STATUS.md +
docs/release-handoff.md; CLASS 3 closure territory
deferred to Step 6) + closure commit Step 6
(final integration pass and track closure; **NO-
BUMP** `pyproject.toml` `version=0.5.2` preserved
byte-identical; этот commit). Track N закрыл
следующий честный продуктовый gap: у платформы уже
были рабочие MCP entrypoints + HTTP/stdio
transports + bearer auth + installer integrity +
deployment-boundary recipe + real MCP client smoke
proof + service-supervision recipe + systemd
template + packaging boundary + buildable wheel,
но не было first-class operator-facing
observability/diagnostics boundary document —
Track J / Track L / Track M каждый denying "full
observability stack" в своём scope; никто не
определял позитивно, что supported diagnostic
surface IS. Step 4 recipe позитивно классифицирует
existing signals в three categories (first-class
supported / recommended-only / out-of-scope),
наследует Track L §9.3 level-to-event mapping
verbatim, документирует de-facto three-bucket
exit-code convention, summarizes Track H §8.4 HTTP
auth-failure-equivalence envelope, carries forward
Track J §6 `/healthz` non-shipping verbatim, и
provides triage recipe с тремя mandatory canonical
failure modes (T1 startup-code-2 / T2 universal-
401 / T3 selfcheck-FAIL).

**Q6 = NO-BUMP** (`0.5.2` preserved). Защита решения
(grounded в repo facts):

- **PATH A docs-only.** Step 4 ship'нул ровно один
  новый файл `docs/operators/observability.md`;
  ноль production code change; ноль `pyproject.toml`
  change; ноль `scripts/*` change.
- **No new declared surface.** No new CLI flag on
  existing servers (`--transport`, `--config-path`,
  `--log-level`, `--bind`, `--auth-token-env`
  surface locked); no new `[project.scripts]`
...
- **No defect-class repair.** Track I PATCH
  ...
  (`pyproject.toml` 11-element packages flip +7
  LOC) required a **declared-but-non-functional**
  wheel-build being made functional. Track N имеет
  **ноль** broken surfaces to repair — existing
  diagnostic signals already work; recipe
  **documents** them, does **not** make them work.
- **Track J / Track K / Track L NO-BUMP precedents
  apply directly.** Track J = PATH A docs-only
  deployment-boundary recipe → NO-BUMP. Track K =
  single new diagnostic file under `scripts/dev/`
  → NO-BUMP. Track L = docs + one declarative
  systemd unit template → NO-BUMP. **Track N —
  purest-form Track J analogue**: pure docs-only
  operator-facing recipe, no template, no helper,
  no code.
- **Step 3 contract §13.5 / §11.4 explicitly framed
  NO-BUMP** as the default expectation under PATH
  A, explicitly prohibited PATCH under PATH A (no
  code change occurs), и explicitly prohibited
  MINOR under any path (guardrails §1.4 — no new
  CLI flag, no new declared surface). MAJOR
  forbidden by track scope.
- **SemVer §6 framing.** PATCH = backward-compatible
  bug fixes. Track N Step 4 = backward-compatible
  **documentation of existing behaviour**, not a
  bug fix. Therefore NO-BUMP is the only honest
  outcome.
...
Mimir / Cortex / VictoriaMetrics platform);
никакого SIEM/SOC integration (no Splunk
forwarder, no Elastic ingestion, no SOAR
scaffolding); никакого distributed tracing;
никакого alerting/paging/on-call workflow;
никакого `/healthz` / `/readyz` / `/livez`
endpoint (Track J §6 defer preserved); никакого
log-aggregation forwarder bundled; никакого
structured-logging library dependency; никаких new
MCP tools; никакого registry change; никаких
transport / auth / deployment-boundary / service-
supervision / packaging redesigns; никакого 1cv8
work; никакого remote push. Stdio transport
runtime byte-identical к Track G / Step 4; HTTP
transport runtime byte-identical к Track H / Step
4 (с Track I defect-fix layered on top); installer
...
preserved). `apps/platform/src/onec_platform/` +
`packages/*/src/` byte-identical к Track M closure
state `a3bdc48`. Это **не** "observability solved
forever" / "production-ready observability" /
"full OpenTelemetry instrumentation" / "Prometheus
platform shipped" / "distributed tracing ready" /
"SIEM-ready" / "enterprise-ready observability" /
"alerting solved" / "all signals covered" claim —
Track N's closure-gate covers **только** одну
integration-and-naming slice (operator-facing
recipe documenting existing signals); broader
observability matrices remain recommended-only.
...
separate JSON-line emitter path); narrow
`selfcheck --json` mode track (post-Track-N
follow-up: PATH B-style single-file slice for
operators who want machine-readable selfcheck
output); `/healthz` / `/readyz` HTTP endpoint
track (carefully gated; Track J §6 defer reversal
would be a separate decision); Windows / macOS
implementation-covered service-supervision-
extension track (post-Track-L follow-up: pywin32
service wrapper + launchd `.plist` artefact);
broader packaging ecosystem track (`.msi` / `.deb`
/ `.rpm` / `.dmg` / signed distribution / PyPI
publication / multi-package-manager publication);
...
OIDC / RBAC / SAML / SCIM / multi-tenant). Эти
кандидаты — recommendation only, не auto-opened.
...
- **Track L closure итог.** Двенадцать post-phase
  ...
  auto-opened. **Историческая правка (Track M /
  Step 1):** "full packaging ecosystem track" из
  списка recommended-next-track candidates был
  частично выбран как следующий открываемый трек
  — открыт как **Track M — Packaging Ecosystem and
  Distribution Boundary** (на момент написания этой
  правки Track M открыт только на Step 1,
  planning, docs-only; полное closure Track M —
  Step 6; broader-packaging-ecosystem parts из
  recommended-list (`.msi` / `.deb` / signed
  distribution / GUI installer / wizard) explicitly
  оставлены за пределами scope как отдельные
  potential tracks — Track M sticks к одной narrow
  distribution-boundary slice). См. секцию
  «Parallel Track M / Step 1 — planning packaging
  ecosystem and distribution boundary (завершён)»
  ниже для подробностей.

### Parallel Track M / Step 1 — planning packaging ecosystem and distribution boundary (завершён)

- **Цель.** Открыть тринадцатый post-phase parallel
  track — Track M — как **planning-only**
  документационный шаг, который закрывает следующий
  честный продуктовый gap: у платформы уже есть
  рабочие MCP entrypoints (Track G), HTTP/stdio
  transports (Tracks G/H), bearer auth (Track H),
  installer integrity (Track I), deployment-
  boundary recipe (Track J), real MCP client smoke
  proof (Track K), и service-supervision recipe +
  systemd template (Track L), но у неё всё ещё нет
  взрослой packaging/distribution story. Step 1 —
  два planning-документа без code changes; Step 4
  design-question (docs-only PATH A vs narrow
  `pyproject.toml` wheel-build flip PATH B vs
  operator-bundle artefact PATH C) **остаётся
  открытым** до Step 2 audit + Step 3 contract.
- **Что shipped в Step 1.**
  - `docs/architecture/track-m-packaging-ecosystem-and-distribution-boundary-plan.md` —
    14-секционный planning-документ: §1 purpose /
    why track exists, §2 current post-Track-L
    baseline (existing `pyproject.toml` declared
    surface, existing release-side scripts, what is
    not in repo today), §3 honest gap statement
    (four independently verifiable observations),
    §4 why gap not already solved (rejection of
    five candidate "we already have this"
    arguments), §5 goal of the track, §6 in-scope,
    §7 out-of-scope (21 explicit denials),
    §8 guardrails (14 hard invariants), §9
    acceptance criteria for eventual closure (11
    items), §10 honest constraints after closure,
    §11 relationship to Tracks G/H/I/J/K/L table,
    §12 Q1–Q7 open questions с directional
    recommendations only (no fake certainty), §13
    step trajectory preview table, §14 honest
    summary.
  - `docs/architecture/track-m-packaging-ecosystem-and-distribution-boundary-step-map.md` —
    6-step map в формате «Goal / What changes /
    What does NOT change / Result» для каждого
    шага; track-invariants block с 21 hard
    constraints включая Track L deliverables byte-
    identical, no new CLI flag, no new
    `[project.scripts]` entries, no new
    dependencies; hard out-of-scope list с 21
    categorical denials; Step 4 explicitly
    preserves PATH A / B / C openness; Step 6 —
    Q7 framing с NO-BUMP / PATCH / MINOR /
    MAJOR-forbidden conditions.
- **Q1–Q7 directional defaults (plan §12).**
  - Q1 = closure-gate target = documented
    distribution-boundary recipe + at most one
    declarative buildable artefact (default
    expectation: PATH B if Step 2 audit confirms
    feasibility without ecosystem expansion).
  - Q2 = installation story included (build
    artefact + documented operator install procedure
    mandatory for closure; build artefact alone
    insufficient).
  - Q3 = default artefact class = **single wheel**
    (`.whl`) buildable via `python -m build` after
    narrow `pyproject.toml` `packages = [...]` flip.
    sdist / operator-bundle acceptable as Step 3
    alternatives.
  - Q4 = cross-OS neutral (Python wheel platform-
    neutral; OS-specific tooling notes prose only).
  - Q5 = Track L service files cross-referenced but
    not bundled inside the Track M artefact.
  - Q6 = `pyproject.toml` change = narrow flip of
    `[tool.hatch.build.targets.wheel] packages = []`
    к populated list (Step 3 PATH B); no
    `[project.urls]` / classifiers / keywords /
    `MANIFEST.in`.
  - Q7 = **NO-BUMP** default если Step 4 PATH A;
    **PATCH** default если Step 4 PATH B (closing
    the long-standing Track C / Step 3 honest
    constraint about the empty wheel build —
    legitimate defect-class repair framing); MINOR
    considered only если Step 4 ship'нет net-new
    operator-facing capability beyond making
    existing declarations functional (rejected by
    Q4 / step-map invariant #15); MAJOR forbidden
    by track scope.
- **Что Step 1 НЕ делал.**
  - Не правил production-код (никаких изменений в
    `apps/*/src/`, `packages/*/src/`).
  - Не правил `pyproject.toml`
    (`version=0.5.1` preserved; wheel-build empty
    preserved per Track C / Step 3 honest constraint).
  - Не открывал и не правил Step 2 audit / Step 3
    contract / Step 4 implementation.
  - Не менял registries (`read=15 / write=25 /
    intelligence=16` invariant carried through;
    никаких новых MCP tool'ов в Step 1).
  - Не правил `SECURITY.md`,
    `docs/release-handoff.md`,
    `apps/platform/README.md`,
    `scripts/release/README.md`, `scripts/dev/README.md`,
    `CHANGELOG.md`, manuals, Closed parallel tracks
    list (Track M не moved туда — Track M active).
  - Не правил `docs/operators/deployment-boundary.md`
    (Track J artefact), `docs/operators/service/*`
    (Track L artefacts), или
    `scripts/dev/mcp_client_smoke.py` (Track K
    artefact) — все byte-identical.
  - Не запускал `1cv8.exe` (Track M работает на
    packaging / distribution layer, не на 1cv8
    binary surface).
  - Не вводил никаких реальных credentials в repo /
    docs / commit message.
  - Не делал remote push / GitHub release —
    operator action, не часть трека.
  - Не открывал Track M Step 2 в этом же commit'е.
  - Не использовал premature closure language —
    Track M framed как **active** (Step 1
    in-progress, Step 2–6 впереди); фразы
    "packaging solved forever" / "PyPI release
    ready" / "signed binary distribution" / "all
    package managers supported" / "production-
    ready packaging" / "enterprise-ready packaging"
    появляются в touched docs **только** как
    explicit DENIALS.
- **Документ scope.** Ровно четыре файла: 2 новых
  planning-документа в `docs/architecture/` +
  README.md (Quickstart paragraph дополнен Track M
  active narrative; Active parallel track section
  reopened) + PROJECT-STATUS.md (header rewrite +
  this section).
- **Verify-release.ps1 -AllowDirtyTree.** GREEN на
...
- **Selfcheck после Step 1.** Зелёный: registries
  ...
  drift'а; selfcheck_status=ok; никаких реальных
  credentials в Step 1 diff'е.
- **Следующий шаг.** **Parallel Track M / Step 2 —
  baseline audit of current packaging / distribution
  state (docs-only).** Новый short audit-документ
  с inventory `pyproject.toml` declared surface
  (`[project]` metadata, `[project.scripts]`
  declarations, empty wheel-build with Track C
  honest-constraint comment block); inventory
  `scripts/release/` artefacts; inventory release-
  handoff prose; whole-repo grep results для
  packaging-vocabulary patterns (`wheel` / `sdist` /
  `dist/` / `release/` / `MANIFEST.in` / `setup.py`
  / `.msi` / `.deb` / `.rpm` / `.dmg` / `.pkg` /
  `signing` / `PyPI` / `Chocolatey` / `Homebrew` /
  `cosign` / `sigstore` / `SBOM` / etc.); 4-class
  breakdown; directional Q1–Q6 resolutions с
  evidence-grounded recommendations + Step 3
...
- **Track M closure итог.** Тринадцать post-phase
  ...
  **Recommended-next-track candidates
  ...
  кандидаты — recommendation only, не auto-opened.
  **Историческая правка (Track N / Step 1):** "полный
  observability stack track (OpenTelemetry /
  Prometheus / log aggregation)" из списка
  recommended-next-track candidates был частично
  выбран как следующий открываемый трек — открыт как
  **Track N — Observability and Diagnostics Boundary**
  (на момент написания этой правки Track N открыт
  только на Step 1, planning, docs-only; полное
  closure Track N — Step 6). Важное scope-уточнение:
  Track N **намеренно намного уже** того, что фраза
  "full observability stack track" в этом списке
  подразумевала. Полный OpenTelemetry program,
  Prometheus/OpenMetrics rollout, Grafana/Tempo/Loki/
  Jaeger platform, SIEM/SOC integration, distributed
  tracing, alerting/paging/on-call workflows, log-
  aggregation forwarder, structured-logging library
  rollout — **explicitly остаются за пределами scope
  Track N** как отдельные potential tracks. Track N
  sticks к одной narrow observability/diagnostics-
  boundary slice (документация supported diagnostic
  surface, опционально один narrow ≤ 200 LOC
  stdlib-only code slice). См. секцию «Parallel
  Track N / Step 1 — planning observability and
  diagnostics boundary (завершён)» ниже для
  подробностей.

### Parallel Track N / Step 1 — planning observability and diagnostics boundary (завершён)

- **Цель.** Открыть четырнадцатый post-phase parallel
  track — Track N — как **planning-only**
  документационный шаг, который закрывает следующий
  честный продуктовый gap: у платформы уже есть
  рабочие MCP entrypoints (Track G), HTTP/stdio
  transports (Tracks G/H), bearer auth (Track H),
  installer integrity (Track I), deployment-boundary
  recipe (Track J), real MCP client smoke proof
  (Track K), service-supervision recipe + systemd
  template (Track L), и packaging/distribution
  boundary + buildable wheel (Track M), но у неё
  всё ещё нет first-class observability /
  diagnostics boundary — нет единого operator-facing
  document, который позитивно называет, какие из
  существующих ad-hoc diagnostic surfaces (stderr на
  трёх MCP server entrypoints, exit codes,
  `scripts/dev/selfcheck.py` single-line status +
  structured summary, `scripts/release/verify-release.ps1`
  8-check gate output, `scripts/dev/mcp_client_smoke.py`
  structured progress lines, HTTP transport auth-
  failure response `401 + WWW-Authenticate: Bearer
  realm="mcp" + JSON-RPC -32001`, install fast-path
  output с password-position redaction по Track D,
  Track L systemd integration через `systemctl
  status` / `journalctl -u`) первоклассные supported
  diagnostic signals, какие recommended-only, и что
  именно продукт **не** обещает (no full OTel / no
  Prometheus / no Grafana / no SIEM / no distributed
  tracing / no alerting / no on-call / no
  `/healthz`). Track J / Track L / Track M каждый
  denying "full observability stack" в своём scope —
  никто не определял позитивно, что supported
  diagnostic surface IS. Step 1 — два planning-
  документа без code changes; Step 4 design-question
  (docs-only PATH A vs. narrow code-bearing
  diagnostic slice PATH B vs. narrow log-shape
  contract slice PATH C) **остаётся открытым** до
  Step 2 audit + Step 3 contract.
- **Что shipped в Step 1.**
  - `docs/architecture/track-n-observability-and-diagnostics-boundary-plan.md` —
    14-секционный planning-документ: §1 purpose /
    why track exists, §2 current post-Track-M
    baseline (existing diagnostic surfaces §2.1,
    what is not in repo today §2.2, what Track N
    therefore must close §2.3), §3 honest gap
    statement (пять independently verifiable
    observations: нет central observability
    document, нет log-shape contract, нет
    documented triage recipe, нет machine-readable
    health signal documented, нет central non-goals
    document), §4 why gap not already solved
    (rejection of шести candidate "we already have
    this" arguments: selfcheck IS diagnostic
    surface / verify-release IS diagnostic surface /
    mcp_client_smoke IS diagnostic surface / stderr
    IS observability / Track L recipe already
    covers this / operators write their own
    observability / Track J/L/M already deny full
    observability), §5 goal of the track, §6
    in-scope, §7 out-of-scope (32 explicit denials
    включая full OTel program / Prometheus /
    Grafana-Tempo-Loki-Jaeger / SIEM / distributed
    tracing / alerting / web UI / `/healthz` / log-
    aggregation forwarder / structured-logging
    library / log-level redesign / new CLI flag /
    new entrypoint module / new dependencies),
    §8 guardrails (22 hard invariants), §9
    acceptance criteria for eventual closure (11
    items), §10 honest constraints after closure,
    §11 relationship to Tracks G/H/I/J/K/L/M
    table, §12 Q1–Q7 open questions с directional
    recommendations only (no fake certainty),
    §13 step trajectory preview table, §14 honest
    summary.
  - `docs/architecture/track-n-observability-and-diagnostics-boundary-step-map.md` —
    6-step map в формате «Goal / What changes /
    What does NOT change / Result» для каждого
    шага; track-invariants block с 25 hard
    constraints включая Tracks A–M deliverables
    byte-identical, Track K diagnostic harness
    byte-identical, Track J deployment-boundary
    recipe byte-identical, Track L service-
    supervision recipe + systemd template byte-
    identical, Track M distribution-boundary recipe
    + wheel-build flip byte-identical (11 src-
    layout packages array preserved); hard out-of-
    scope list с 27+ categorical denials; Step 4
    explicitly preserves PATH A / B / C openness;
    Step 6 — Q6 framing с NO-BUMP / PATCH /
    MINOR-forbidden conditions.
- **Q1–Q7 directional defaults (plan §12).**
  - Q1 = что считается "observability/diagnostics"
    для closure → **PATH A docs-only** как
    narrowest honest closure (PATH B / PATH C
    considered fallback per Step 2 audit).
  - Q2 = diagnostic artefact only vs. triage story
    too → **(B) triage story included** as
    operator-facing closure target (3–5 canonical
    failure modes end-to-end MUST cover); recipe
    alone qualifies as Track N closure even при
    PATH A.
  - Q3 = artefact class в Step 4 → **(A) no
    artefact** как narrowest closure path; (B)
    `selfcheck --json` структура narrow slice как
    fallback if Step 2 audit reveals strong
    machine-readable-signal honest gap; (C) log-
    shape contract slice rejected by default
    (touches all three MCP server entrypoints); (D)
    diagnostic-bundle helper rejected by default
    (duplicates Track K / Track L surfaces).
  - Q4 = cross-OS vs. one primary observation OS →
    **(B) Linux/systemd/journald primary**,
    симметрично Track L closure target; cross-OS
    prose для Windows (NSSM / Event Viewer) и
    macOS (launchd / Console.app); recipe SHOULD
    extend Track L's `journalctl -u mcp-server`
    surface into documented signal vocabulary.
  - Q5 = current selfcheck / verify-release / client
    smoke в supported diagnostics surface → **(A)
    all three first-class**, each с named role
    (`selfcheck.py` = pre-flight gate /
    `verify-release.ps1` = release-side gate /
    `mcp_client_smoke.py` = transport-boundary
    smoke); zero code cost.
  - Q6 = NO-BUMP / PATCH / MINOR → **(A) NO-BUMP**
    if Step 4 PATH A; PATCH considered только если
    Step 4 PATH B/C ships honest defect-class
    diagnostic-surface repair (мирror Track I /
    Track M PATCH precedents); MINOR explicitly
    rejected by guardrails §8.5 (no new CLI flag),
    §8.6 (no new entrypoint module), §8.7 (no new
    dependencies); MAJOR forbidden by track scope.
  - Q7 = как избежать silent expansion в full
    observability platform → **(C) both denial
    discipline + numeric caps** simultaneously
    locked through every step (explicit denial of
    OTel / Prometheus / Grafana / Tempo / Loki /
    Jaeger / SIEM / distributed tracing / alerting /
    on-call в каждом track doc + Step 3 contract
    pins ≤ 3 files Step 4 file-surface cap + ≤ 200
    LOC stdlib-only LOC cap + no new dependency +
    no new entrypoint module + no new CLI flag).
- **Что Step 1 НЕ делал.**
  - Не правил production-код (никаких изменений в
    `apps/*/src/`, `packages/*/src/`).
  - Не правил `pyproject.toml` (`version=0.5.2`
    preserved от Track M closure bump; wheel-build
    `packages = [...]` 11-element array preserved
    byte-identical к Track M / Step 4 flip).
  - Не открывал и не правил Step 2 audit / Step 3
  ...
  - Не правил `SECURITY.md`,
    ...
    `apps/platform/README.md`,
    `scripts/release/README.md`,
    `scripts/dev/README.md`, `CHANGELOG.md`,
    manuals, Closed parallel tracks list (Track N
    не moved туда — Track N active).
  - Не правил `docs/operators/deployment-boundary.md`
    (Track J artefact), `docs/operators/service/*`
    (Track L artefacts), или
    `docs/operators/packaging/distribution-boundary.md`
    (Track M artefact) — все byte-identical.
  - Не запускал `1cv8.exe` (Track N работает на
    observability/diagnostics-documentation layer,
    не на 1cv8 binary surface).
  - Не вводил никаких реальных credentials в repo /
    docs / commit message.
  - Не делал remote push / GitHub release —
    operator action, не часть трека.
  - Не открывал Track N Step 2 в этом же commit'е.
  - Не использовал premature closure language —
    Track N framed как **active** (Step 1
    in-progress по уровню Track, Step 1 сам по себе
    завершён; Step 2–6 впереди); фразы
    "observability solved forever" / "production-
    ready observability" / "full OTel
    instrumentation" / "Prometheus platform
    shipped" / "distributed tracing ready" /
    "SIEM-ready" / "enterprise-ready observability"
    / "alerting solved" появляются в touched docs
    **только** как explicit DENIALS.
  - Не делал false implementation claims — Q1–Q7
    presented как **defaults** / **directional
    recommendations**, не как decided answers;
    Step 3 contract — место для locking.
- **Документ scope.** Ровно четыре файла: 2 новых
  planning-документа в `docs/architecture/` +
  README.md (Quickstart paragraph дополнен Track N
  active narrative; "Active parallel track" section
  reopened с Track N описанием) + PROJECT-STATUS.md
  (header rewrite + эта section). Track M closure
  block внизу этого статус-документа preserved
  byte-identical.
- **Verify-release.ps1 -AllowDirtyTree.** GREEN на
  8 checks; никаких реальных credentials в Step 1
...
- **Selfcheck после Step 1.** Зелёный: registries
  ...
  drift'а; `selfcheck_status=ok`; никаких реальных
  credentials в Step 1 diff'е.
- **Следующий шаг.** **Parallel Track N / Step 2 —
  baseline audit of current observability /
  diagnostics state (docs-only).** Новый descriptive
  audit-документ
  (`docs/architecture/track-n-observability-and-diagnostics-boundary-baseline-audit.md`)
  с inventory stderr emission across трёх MCP
  server entrypoints (which startup banner lines
  emitted, где в code, with what shape); inventory
  exit-code paths (SystemExit / KeyboardInterrupt /
  unhandled-exception / config-load-failure /
  auth-config-failure); inventory selfcheck.py
  output shape; inventory verify-release.ps1 output
  shape; inventory mcp_client_smoke.py output
  shape; inventory HTTP transport auth-failure
  responses (Track H 401 + WWW-Authenticate +
  JSON-RPC -32001); inventory install fast-path
  output shape (Track D password-position
  redaction); inventory Track L systemd
  integration observation surface; whole-repo grep
  results для observability-vocabulary patterns
  (`log` / `logging` / `INFO` / `WARN` / `ERROR` /
  `DEBUG` / `--log-level` / `selfcheck` / `verify` /
  `triage` / `troubleshoot` / `diagnostic` /
  `health` / `monitoring` / `metrics` / `tracing` /
  `prometheus` / `otel` / `opentelemetry` /
  `jaeger` / `tempo` / `loki` / `grafana` / `siem`);
  4-class breakdown (already-reusable / adjacent-
  but-insufficient / clearly-missing / explicitly-
  out-of-scope); directional Q1–Q5 resolutions с
  evidence-grounded recommendations + Step 3
...
### Parallel Track N / Step 6 — final integration pass and track closure (завершён)

- **Цель.** Final integration pass над Track N
  Steps 1–5 + честное Q6-решение + closure
  narrative в README / PROJECT-STATUS / CHANGELOG.
  Закрыть весь Track N. Active tracks remaining =
  none после Step 6.
- **Q6 = NO-BUMP** (`0.5.2` preserved byte-
  ...
  ноль `pyproject.toml` change; ноль new declared
  surface; ноль defect-class repair (recipe
  documents existing signals — does **not** make
  them work); Track J/K/L NO-BUMP precedents
  apply directly; Step 3 contract §13.5 / §11.4
  explicitly framed NO-BUMP default под PATH A и
  prohibited PATCH (no code change occurs) +
  MINOR (no new declared surface); MAJOR forbidden
  by track scope; SemVer §6 framing: PATCH =
  backward-compatible bug fixes; Track N =
  backward-compatible **documentation of existing
  behaviour**, not a bug fix; NO-BUMP is the only
  honest outcome.
- **Closure scope (narrowest honest).** Touched:
  `README.md` (Quickstart blockquote summary
  flipped от active Track N к Track N closure
  narrative; Closed parallel tracks list extended
  от тринадцать до **четырнадцать** с Track N
  entry; Active parallel track section compressed
  back к no-active-track wording; new "Track N
  detail (закрыт)" section added above "Track M
  detail (закрыт)"); `PROJECT-STATUS.md` (header
  rewritten от "Track N / Step 5 active" к "no
  active step + Track N fully closed под `0.5.2`";
  ...
  `CHANGELOG.md` (Track N closure narrative added
  under existing `## 0.5.2 — Parallel Track M`
  section с explicit "Track N closed under
  existing 0.5.2 without further bump" framing).
  **NOT touched:** `pyproject.toml`
  (`version="0.5.2"` preserved byte-identical —
  NO-BUMP Q6 decision); `SECURITY.md`;
  `docs/release-handoff.md` (Step 5 уже выровнял);
  `apps/platform/README.md`;
  `docs/operators/observability.md` (Step 4
  deliverable immutable); `docs/operators/
  deployment-boundary.md` (Track J artefact);
  `docs/operators/service/service-supervision.md`
  и `mcp-server.service` (Track L artefacts);
  `docs/operators/packaging/distribution-
  boundary.md` (Track M artefact); все `scripts/*`;
  `scripts/dev/README.md`; `scripts/release/
  README.md`; Track N Step 1/2/3 architecture
  docs (frozen anchors); production code;
  `apps/*/src/`; `packages/*/src/`; manuals;
  examples; CI workflow; `LICENSE`.
- **Verify-release.** GREEN на 8 checks pre-commit
  (`-AllowDirtyTree`) и post-commit (clean tree);
  selfcheck registries `read=15 / write=25 /
  intelligence=16` без drift'а; никаких реальных
  credentials в closure commit'е; никаких 1cv8.exe
  runs; никакого remote push.
- **Track N closure итог.** **Четырнадцать** post-
  phase parallel track'ов (A, B, C, D, E, F, G,
  H, I, J, K, L, M, N) полностью закрыты. Phase 7
  как линейная фаза не запланирована. Открытие
  следующего параллельного трека — отдельное
  operator decision. **Recommended-next-track
  candidates (recommendation only, без
  автоматического открытия):** structured-logging
  library adoption track (post-Track-N follow-up:
  `structlog` / `python-json-logger` dependency +
  per-record key vocabulary + JSON-line emitter
  path); narrow `selfcheck --json` mode track
  (post-Track-N follow-up); `/healthz` /
  `/readyz` HTTP endpoint track (Track J §6 defer
  reversal would be separate decision); Windows /
  ...
  expansion (separate enterprise-grade identity
  track); enterprise identity stack track (SSO /
  OIDC / RBAC / SAML / SCIM / multi-tenant);
  editable-install / workspace-discovery dev-time
  tooling track. Эти кандидаты — recommendation
  only, не auto-opened. **Историческая правка
  (Track O / Step 1):** "editable-install /
  workspace-discovery dev-time tooling track" из
  списка recommended-next-track candidates был
  выбран как следующий открываемый трек — открыт
  как **Track O — Dev-Time Editable Install and
  Workspace Discovery** (на момент написания этой
  правки Track O открыт только на Step 1,
  planning, docs-only; полное closure Track O —
  Step 6). Track O намеренно узкий: closes
  dev-time editable install / workspace discovery /
  bootstrap boundary for contributors editing the
  repo in place, **orthogonal to Track M's deploy-
  time wheel distribution boundary**. Other
  recommended candidates above (structured-logging
  library, `selfcheck --json`, `/healthz`,
  Windows/macOS service-supervision-extension,
  broader packaging ecosystem, multi-version 1С,
  rollback/AST, web UI, TLS-in-process/mTLS,
  enterprise identity) explicitly остаются за
  пределами scope Track O как отдельные potential
  tracks. См. секцию «Parallel Track O / Step 1 —
  planning dev-time editable install and workspace
  discovery (завершён)» ниже для подробностей.

### Parallel Track O / Step 1 — planning dev-time editable install and workspace discovery (завершён)

- **Цель.** Открыть пятнадцатый post-phase parallel
  track — Track O — как **planning-only**
  документационный шаг, который закрывает следующий
  честный продуктовый gap: после Track M ввёл узкий
  supported deploy-time wheel distribution boundary
  ...
  за пределами PowerShell-only
  `scripts/dev/bootstrap_paths.ps1`, нет cross-OS
  bootstrap parity. Gap явно acknowledged в репо:
  ...
  marker. Track O — dedicated narrow track,
  **orthogonal to Track M**: deploy-time wheel
  distribution остаётся в scope Track M; dev-time
  editable workflow для contributors — новая,
  отдельная axis. Step 1 — два planning-документа
  без code changes; Step 4 design-question (docs-
  only PATH A vs. narrow declarative slice PATH B
  vs. bootstrap helper PATH C) **остаётся
  открытым** до Step 2 audit + Step 3 contract.
- **Что shipped в Step 1.**
  - `docs/architecture/track-o-dev-time-editable-install-and-workspace-discovery-plan.md` —
    14-секционный planning-документ: §1 purpose /
    why track exists; §2 current post-Track-N
    baseline (existing dev-time surfaces: bootstrap_
    paths.ps1, launch.ps1, run_dev_check.ps1,
    selfcheck.py, mcp_client_smoke.py, scripts/dev/
    README.md, pyproject.toml hatchling build, .python-
    version Python 3.11 pin, CI workflow; what is
    NOT in repo today); §3 honest gap statement
    (пять independently verifiable observations,
    включая verbatim quote of `scripts/dev/
    README.md:5-11` explicit out-of-scope sentence);
    §4 why gap NOT already closed by Track M (пять
    rejections); §5 goal of the track; §6 in-scope;
    §7 out-of-scope (23+ explicit denials including
    no containerised dev, no IDE integrations, no
    remote-dev, no multi-Python-version matrix, no
    formatter/linter/test-runner redesign, no
    alternative build-backend); §8 guardrails (22
    hard invariants); §9 acceptance criteria (11
    items); §10 honest constraints after closure;
    §11 relationship to Tracks G/H/I/J/K/L/M/N
    table; §12 Q1–Q7 open questions с directional
    recommendations only; §13 step trajectory
    preview table; §14 honest summary.
  - `docs/architecture/track-o-dev-time-editable-install-and-workspace-discovery-step-map.md` —
    6-step map в формате «Goal / What changes / What
    does NOT change / Result» для каждого шага;
    track-invariants block с 31 hard constraints
    включая Tracks A–N deliverables byte-identical,
    no new MCP tools, no new CLI flag, no new
    `[project.scripts]` entries, no new dependencies,
    no new entrypoint module, `.python-version`
    preserved, CI workflow preserved; hard out-of-
    scope list с 25+ categorical denials; Step 4
    explicitly preserves PATH A/B/C openness; Step
    6 — Q7 framing с NO-BUMP default под PATH A,
    PATCH acceptable только под PATH B с honest
    defect-class declarative repair, MINOR
    explicitly prohibited, MAJOR forbidden by track
    scope.
- **Q1–Q7 directional defaults (plan §12).**
  - Q1 = closure-gate target → **(A) one supported
    dev-time workflow recipe** as narrowest honest
    closure; PATH B (recipe + narrow declarative
    slice) acceptable fallback; PATH C (recipe +
    bootstrap helper) rejected by default.
  - Q2 = primary problem focus → **(A) narrow
    contributor-facing dev-time editable install +
    workspace discovery + bootstrap boundary**;
    (B) installable-from-git-URL rejected;
    (C) broader DX policies rejected.
  - Q3 = Step 4 PATH openness → **PATH A docs-only
    primary**, PATH B held in reserve; **Step 4
    PATH not locked at Step 1**; Step 3 contract
    locks.
  - Q4 = minimum closure scope = mandatory answers
    to: how developer installs from checkout;
    whether `pip install -e .` is supported; how
    workspace discovery works (which src-layout
    paths are dev-time PYTHONPATH entries vs.
    wheel-build `packages` entries — currently the
    same 11, per Track M); what is first-class vs.
    recommended-only vs. out-of-scope.
  - Q5 = insufficient closure proof: "just clone
    the repo" / "`pip install -e .` maybe works" /
    ad-hoc tribal knowledge / scattered comments /
    generic folklore.
  - Q6 = production code change → likely **NOT
    required**; declarative / dev-tooling / docs
    boundary; Step 2 audit must verify.
  - Q7 = SemVer expectation → **(A) NO-BUMP** если
    Step 4 PATH A docs-only mirroring Track J/K/L/N
    NO-BUMP precedent; (B) PATCH considered только
    если Step 4 PATH B with honest defect-class
    declarative repair closing `scripts/dev/
    README.md:5-11` out-of-scope sentence (mirror
    of Track I / Track M PATCH precedents);
    (C) MINOR explicitly rejected; MAJOR forbidden
    by track scope.
- **Что Step 1 НЕ делал.**
  - Не правил production-код (никаких изменений в
    `apps/*/src/`, `packages/*/src/`).
  - Не правил `pyproject.toml` (`version=0.5.2`
    preserved; wheel-build `packages = [...]`
    11-element array preserved byte-identical к
    Track M / Step 4 flip; `[tool.ruff]` и
    `[tool.pytest.ini_options]` byte-identical;
    `[project.dependencies]` пустой preserved).
  - Не открывал и не правил Step 2 audit / Step 3
    contract / Step 4 implementation / Step 5
    alignment / Step 6 closure.
  - Не менял registries (`read=15 / write=25 /
  ...
  - Не правил `scripts/dev/selfcheck.py`,
    ...
    `scripts/release/_install_runner.py`,
    `scripts/release/verify-release.ps1`,
    `scripts/release/README.md` — все byte-
    identical.
  - Не правил `SECURITY.md`,
    `docs/release-handoff.md`,
    `apps/platform/README.md`,
    `CHANGELOG.md`, manuals (`docs/operator-
    manual.md` / `docs/administrator-manual.md` /
    `docs/developer-manual.md`), Closed parallel
    tracks list (Track O не moved туда — Track O
    active; still 14 entries A–N).
  - Не правил existing operator recipes
    (`docs/operators/deployment-boundary.md` —
  ...
  - Не правил `.python-version` — Python 3.11 pin
    preserved.
  - Не запускал `1cv8.exe` (Track O работает на
    dev-time tooling / documentation layer, не на
    1cv8 binary surface).
  - Не вводил никаких реальных credentials.
  - Не делал remote push / GitHub release —
    operator action, не часть трека.
  - Не открывал Track O Step 2 в этом же commit'е.
  - Не использовал premature closure language —
    Track O framed как **active** (Step 1
    in-progress по уровню Track, Step 1 сам по
    себе завершён; Step 2–6 впереди); фразы
    "developer workflow solved forever" / "all
    IDE integrations supported" / "all package
    managers supported for dev install" /
    "containerised dev environment shipped" /
    "remote-dev shipped" / "enterprise developer
    experience" появляются в touched docs
    **только** как explicit DENIALS.
  - Не делал false implementation claims — Q1–Q7
    presented как **defaults** / **directional
    recommendations**, не как decided answers;
    Step 4 PATH openness preserved; Step 3
    contract — место для locking.
- **Документ scope.** Ровно четыре файла: 2 новых
  planning-документа в `docs/architecture/` +
  README.md (Quickstart paragraph дополнен Track O
  active narrative; "Active parallel track" section
  reopened с Track O описанием) + PROJECT-STATUS.md
  (header rewrite + эта section). Track N closure
  block внизу этого статус-документа preserved
  byte-identical.
- **Verify-release.ps1 -AllowDirtyTree.** GREEN на
  8 checks; никаких реальных credentials в Step 1
...
- **Selfcheck после Step 1.** Зелёный: registries
  `read=15 / write=25 / intelligence=16` без
  drift'а; `selfcheck_status=ok`.
- **Следующий шаг.** **Parallel Track O / Step 2 —
  baseline audit of current dev-time editable
  install / workspace discovery state (docs-only).**
  Новый descriptive audit-документ
  (`docs/architecture/track-o-dev-time-editable-install-and-workspace-discovery-baseline-audit.md`)
  с inventory of `scripts/dev/*`
  (bootstrap_paths.ps1, launch.ps1,
  run_dev_check.ps1, selfcheck.py,
  mcp_client_smoke.py, README.md); inventory of
  `pyproject.toml` developer-facing surface
  ([build-system], [project], [project.scripts],
  [tool.hatch.build.targets.wheel], [tool.ruff],
  [tool.pytest.ini_options]); inventory of
  `.python-version`; inventory of CI workflow;
  inventory of developer-facing prose в
  `docs/developer-manual.md`; whole-repo grep
  results для dev-vocabulary patterns
  (`pip install -e`, `editable`, `PEP 660`,
  `bootstrap`, `workspace`, `monorepo`,
  `PYTHONPATH`, `develop`, `setup.py develop`);
  inventory of `scripts/dev/README.md:5-11`
  explicit out-of-scope sentence; 4-class
  breakdown (already-reusable / adjacent-but-
  insufficient / clearly-missing / explicitly-out-
  of-scope); directional Q1–Q6 resolutions с
  evidence-grounded recommendations + Step 3
  handoff list (≥ 10 items). Production-код Step 2
  не правит. Никаких real credentials. **GitHub
  remote push — operator action, не часть трека.**
...
### Parallel Track Q / Step 1 — planning Windows installer path and setup.exe delivery (завершён)

- **Цель.** Открыть семнадцатый post-phase
  parallel track — Track Q — как **planning-only**
  документационный шаг, который закрывает
  **другой** честный продуктовый gap, чем Track P:
  у проекта есть wheel/pip distribution boundary
  (Track M: `1c_agent_platform-0.5.2-py3-none-any.whl`,
  `pip install <WHEEL_PATH>`), install fast-path
  PowerShell wrapper (Track B / Track I:
  `scripts/release/install.ps1`), и editable
  install (Track O: `pip install -e .`) — но
  ...
  **Setup.exe path отсутствует.** Конечный
  ...
  нет на HEAD `d6f1936`. Gap явно acknowledged
  тремя независимо проверяемыми anchor'ами:
  `scripts/release/install.ps1:1-11` (umbrella
  comment — "No `.msi`, no `.deb`, no GUI wizard,
  no signed distribution");
  `docs/operators/packaging/distribution-boundary.md`
  §7 non-goals (Track M recipe, committed
  `31313db`) — verbatim "no GUI installer";
  `pyproject.toml` lines 44–50 (Track M comment
  block внутри `[tool.hatch.build.targets.wheel]`)
  — verbatim "no GUI installer", "no Chocolatey /
  Homebrew / apt / conda-forge / NuGet", "no
  enterprise-ready packaging". Эти anchor'ы —
  honest deferrals (не contradictions): non-goals
  Track M scope'а, не permanent closures
  installer-вселенной. Track Q — dedicated
  **narrow** track, который deferral закрывает:
  один Windows `setup.exe`, ordinary-user install
  experience без preinstalled Python / pip / Git,
  uninstall через стандартный Windows
  Add/Remove Programs surface (registration под
  `HKCU\Software\Microsoft\Windows\CurrentVersion\
  Uninstall\<GUID>`). Recommended-next-track
  candidates list (выше в этом документе)
  содержал "broader packaging ecosystem track
  (`.msi` / `.deb` / `.rpm` / `.dmg` / `.pkg` /
  signed distribution chain / PyPI publication /
  multi-package-manager publication)". Track Q
  **не** этот broader candidate — Track Q
  narrowly: Windows только, setup.exe только,
  одна installer technology (Inno Setup default),
  никаких `.msi`/`.deb`/`.rpm`/`.dmg`/`.pkg`/
  `.snap`/`.flatpak` widely, никакой PyPI, никаких
  package-manager матриц, никакого signing'а,
  никакого auto-update'а. Broader candidate
  остаётся available как separate future track;
  Track Q его **не** absorb'ит.
- **Центральная честная константа Track Q
  (плана §4).** Платформа — pure-Python codebase
  (Track M `py3-none-any` wheel содержит eleven
  src-layout packages). При runtime требуется
  CPython 3.11 interpreter. **Acceptance
  criterion "no preinstalled Python / pip / Git"
  структурно требует bundled Python runtime**
  внутри installer'а. Существуют ровно две
  честные опции: **(α) bundled embeddable
  distribution** —
  `python-3.11.<x>-embed-amd64.zip` от python.org
  (~10 MB: `python.exe`, stdlib `.zip`,
  `pythonXY.dll`, minimal `.pyd` modules);
  installer extracts его в install directory;
  **(β) frozen executable distribution** —
  PyInstaller / Nuitka / Briefcase / py2exe
  produces standalone `.exe` files; installer
  ships frozen exes. Нет честной третьей опции.
  **(α)** — default expectation (поскольку (β)
  добавляет PyInstaller/Nuitka toolchain'ы,
  которых сегодня в репо нет). Expected
  installed footprint — около **10–15 MB**
  (~10 MB embeddable CPython + ~1 MB платформа
  + small installer overhead) как структурная
  стоимость закрытия gap'а; никакого reduction
  не promised. Bundled-runtime бинарь
  **не должен** быть committed в git —
  python.org embeddable acquires the build
  helper **at build time**, не at clone time.
  Эта константа названа прямо как §4 в plan'е,
  не в сноске; она central design constraint, не
  parenthetical.
- **Что shipped в Step 1.**
  - `docs/architecture/track-q-windows-installer-path-and-setup-exe-delivery-plan.md`
    — 14-секционный planning-документ: §1 purpose
    / why track exists; §2 current post-Tracks-A–O
    baseline relevant to Track Q (ordinary-Windows-
    user persona reality + inventory three existing
    install-adjacent surfaces
    [`scripts/release/install.ps1`, `pip install
    <WHEEL_PATH>`, `pip install -e .`] и их Python
    prerequisite + что репо НЕ ship'ит: setup.exe,
    bundled Python runtime, installer-technology
    definition file, GitHub Releases setup.exe
    asset, Authenticode signing pipeline, Windows
    Add/Remove Programs registry entry +
    explicit anchor citations
    `scripts/release/install.ps1:1-11`,
    `docs/operators/packaging/distribution-boundary.md
    §7`, `pyproject.toml` Track M comment block); §3
    honest gap statement (шесть независимо
    проверяемых observations); §4 central honest
    constraint — bundled Python runtime
    структурно требуется (shape α vs shape β; нет
    третьей опции; default expectation α
    embeddable CPython 3.11 pulled at build time;
    expected installed footprint 10–15 MB); §5
    goal of the track; §6 in-scope; §7 out-of-
    scope с 30+ explicit denials (no Linux/macOS
    installer / no broad `.msi`/`.deb`/`.rpm`/
    `.dmg`/`.pkg`/`.snap`/`.flatpak` ecosystem /
    no PyPI publication / no package-manager
    matrix [Chocolatey / winget / Scoop / NuGet /
    Microsoft Store / MSIX in store sense] / no
    code signing / Authenticode / EV cert /
    notarization / SBOM / SLSA / supply-chain
    attestation / no auto-update / OTA / delta-
    update / no GUI dashboard / no browser UI /
    web admin panel / no service supervision
    redesign / no auth redesign / no transport
    redesign / no new MCP tools / no registry
    changes / no new CLI flag / no new
    `[project.scripts]` entries / no new
    entrypoint module / no new transports / no
    remote-dev / IDE integration / no enterprise
    installer platform / MDM / Group Policy /
    SCCM / Intune / no containerisation / no
    cluster-HA / "all Windows distributions
    supported" / "one-click everything solved
    forever" / "enterprise-grade installer" /
    "production-ready desktop app" / "GUI config
    wizard" / "Windows service auto-magic by
    default" claims / no test-suite program /
    no observability redesign / no remote push /
    no new parallel track opened within Step 1);
    §8 guardrails (30 hard invariants); §9
    acceptance criteria (12 items); §10 honest
    constraints after closure (no "production-
    ready Windows installer platform" claim;
    bundled-runtime footprint cost; unsigned
    setup.exe by default; no auto-update;
    single-user single-machine; no GUI config
    step; no new MCP tools / registry change; no
    1cv8.exe runs); §11 relationship-to-Tracks-
    M/L/O/P table (Track Q is **Windows only**
    cross-OS axis; **YES — bundled embeddable
    CPython 3.11** runtime axis; double-click
    `setup.exe → Next → Install → Finish` install
    verb axis; **Add/Remove Programs → Uninstall**
    uninstall verb axis; **HKCU\Software\...\
    Uninstall\<GUID>** Windows-registry uninstall
    entry axis); §12 Q1–Q7 open questions с
    directional recommendations only; §13 step
    trajectory preview table; §14 honest summary.
  - `docs/architecture/track-q-windows-installer-path-and-setup-exe-delivery-step-map.md`
    — 6-step map в формате «Goal / What changes /
    What does NOT change / Result» для каждого
    шага; track-invariants block с 35 hard
    constraints включая Tracks A–O production
    code byte-identical, Track K diagnostic
    harness byte-identical, Track J/L/M/N
    operator recipes byte-identical, Track O dev
    recipe byte-identical, **Track P / Step 1
    planning surface byte-identical** (Track Q
    не trogeт Track P deliverables), `pyproject.
    toml` byte-identical Steps 1–5 (Step 6 MAY
    bump `version` только если Q7 = PATCH или
    MINOR), `scripts/dev/*` и `scripts/release/*`
    byte-identical (Track Q не trogeт ни одного
    существующего scripts/-файла; если Step 4
    PATH B/C требует build helper, это new file
    под `installer/`), `.python-version`
    preserved как bundled-runtime target,
    `1c_agent_platform-*.whl` byte-identical
    (installer built **on top of** wheel, не over
    него), **no bundled-runtime binary committed
    to git** (python.org embeddable acquires at
    build time, не at clone time; Step 4 file
    surface MUST NOT включать `.zip`/`.exe`/
    `.dll`/`.pyd`/`.msi`/`.cab`/`.7z`/`.tar.*`/
    `.gz`/`.bz2`/`.xz` или другой binary
    artefact), no new MCP tools, no new CLI flag,
    no new `[project.scripts]` console entries,
    no new project dependencies (runtime или
    optional), no new entrypoint module, no
    1cv8.exe runs, no outbound network at
    runtime, no real credentials, no remote push;
    hard out-of-scope list с 25+ categorical
    denials (carry through every step); Step 4
    explicitly preserves PATH A/B/C openness
    (default expectation PATH B; PATH A в
    резерве; PATH C standalone rejected); Step 6
    — Q7 framing с NO-BUMP defensible under
    PATH A (mirror Track J/K/L/N/O precedent),
    PATCH defensible under PATH B framed as
    defect-class delivery-channel repair (mirror
    Track I / Track M precedents), MINOR
    defensible under PATH B framed as new
    operator-visible delivery channel (mirror
    Track H precedent), MAJOR forbidden by track
    scope.
- **Q1–Q7 directional defaults (план §12).**
  - Q1 = closure target → **один real `setup.exe`
    ...
    installer-definition slice** as most likely
    outcome (mirror Track M / Step 4 closing the
    `packages = []` aspirational declaration);
    **(A) recipe-only** acceptable fallback;
    **(C) standalone build helper без `.iss`**
    rejected by default.
  - Q2 = installer technology default → **(A)
    Inno Setup** baseline (простейшая честная
    Windows installer technology с native
    uninstall registry support; declarative
    `.iss`; free tooling; long reliability track
    record на Windows 10/11); **(B) WiX /
    `.msi`** considered только если Step 2
    surfaces grounded MDM / SCCM / Intune
    operator need; **(C) NSIS** considered только
    если Step 2 surfaces specific need; **(D)
    Advanced Installer / (E) InstallShield**
    rejected by default (commercial); **(F)
    MSIX** rejected by default (Track Q не ship'ит
    в Store). **Step 1 explicitly does NOT lock**
    technology choice; Step 3 contract — lock
    point.
  - Q3 = Step 4 PATH openness → **PATH B**
    (operator recipe + один `.iss` + опционально
    build helper) primary; **PATH A** docs-only в
    резерве; **PATH C** standalone helper без
    `.iss` rejected. **Step 4 PATH not locked at
    Step 1**; Step 3 contract — lock point.
  - Q4 = install experience prerequisite stance →
    install experience **MUST NOT** require
    preinstalled Python (any version) / pip / Git
    / Visual Studio Build Tools / MSVC / Windows
    SDK / любой third-party package manager
    (Chocolatey / winget / Scoop / NuGet) / любой
    C/C++ runtime beyond what Windows 10/11 ships
    by default. **Direct implication**: installer
    **MUST** bundle Python runtime; default
    expectation — python.org embeddable CPython
    3.11 distribution pulled at build time.
  - Q5 = uninstall path stance → uninstall —
    **first-class part of supported boundary**;
    installer **MUST** register uninstall entry
    под
    `HKCU\Software\Microsoft\Windows\CurrentVersion\
    Uninstall\<GUID>` (или per-machine `HKLM`
    equivalent); ordinary Windows "Settings →
    Apps → Installed apps" surface — supported
    entry point uninstall'а; Inno Setup handles
    это natively.
  - Q6 = production code change → **likely NOT
  ...
  - Q7 = SemVer expectation — **directional
    framing only**, lock на Step 6: **(A)
    NO-BUMP** под PATH A docs-only (mirror Track
    J/K/L/N/O NO-BUMP precedent); **(B) PATCH**
    defensible под PATH B framed as defect-class
    delivery-channel repair, закрывающий long-
    standing deferred "no GUI installer" non-
    goal anchor в Track M recipe и `pyproject.
    toml` comment block (mirror Track I / Track M
    PATCH precedents для declarative-defect-
    class repair); **(C) MINOR** defensible под
    PATH B framed as new operator-visible
    delivery channel — ordinary Windows user
    без Python на машине — genuinely new audience
    и new delivery channel что ordinary product
    consumers могут use, что не могли use before
    (mirror Track H MINOR precedent introduction
    network transport как new operator-visible
    delivery capability); **(D) MAJOR** forbidden
    by track scope. Step 1 captures все три
    (NO-BUMP / PATCH / MINOR) как live
    possibilities и locks **ни одну**.
- **Что Step 1 НЕ делал.**
  - Не правил production-код (никаких изменений в
    `apps/*/src/`, `packages/*/src/`).
  - Не правил `pyproject.toml` (`version=0.5.2`
    preserved; packages array preserved
    [Track M / Step 4 lock invariant];
    `[tool.pytest.ini_options]` preserved;
    `[tool.ruff]` byte-identical; `[project.
    dependencies]` empty preserved; `[project.
    scripts]` byte-identical; no `[project.
    optional-dependencies]` block added).
  - Не открывал и не правил Step 2 audit /
    Step 3 contract / Step 4 implementation /
    Step 5 alignment / Step 6 closure.
  - Не выбирал installer technology финально —
    Inno Setup как directional baseline, не
    contract-binding.
  - Не commit'ил installer-definition file
    (`.iss`/`.wxs`/`.nsi`/`.aip` — нет ни одного).
  - Не commit'ил bundled-runtime binary
    (`python-*-embed-*.zip` или другой CPython
    artefact — отсутствует).
  - Не менял registries (`read=15 / write=25 /
    intelligence=16` invariant carried through;
    никаких новых MCP tool'ов).
  - Не правил `scripts/dev/selfcheck.py`,
    ...
    `scripts/dev/bootstrap_paths.ps1`,
    `scripts/dev/run_dev_check.ps1`,
    `scripts/dev/README.md`, all
    `scripts/release/*` (включая
    `scripts/release/install.ps1`, чей
    umbrella comment мы цитируем как anchor —
    он сам byte-identical) — все byte-identical.
  - Не правил `SECURITY.md`, `docs/release-
    handoff.md`, `apps/platform/README.md`,
    `CHANGELOG.md`, manuals, Closed parallel
    tracks list (Track Q не moved туда — Track Q
    active; still 15 entries A–O).
  - Не правил existing operator recipes
    (`docs/operators/deployment-boundary.md` —
    Track J; `docs/operators/service/*` —
    Track L; `docs/operators/packaging/
    distribution-boundary.md` — Track M;
    `docs/operators/observability.md` —
    Track N) — все byte-identical.
  - Не правил Track O dev recipe
    (`docs/dev/editable-install-and-workspace-discovery.md`).
  - **Не правил Track P / Step 1 planning
    surface** —
    `docs/architecture/track-p-test-suite-shipping-and-verification-boundary-plan.md`
    и
    `docs/architecture/track-p-test-suite-shipping-and-verification-boundary-step-map.md`
    byte-identical. Track Q не trogeт Track P
    deliverables; Track P остаётся active в
    своём ритме (Step 2–6 — independent
    operator decisions).
  - Не правил `.github/workflows/dev-check.yml`
    — byte-identical.
  - Не правил `.python-version` — Python 3.11
    pin preserved (как target для bundled
    runtime).
  - Не создавал `installer/windows/` directory —
    создание `.iss` или build helper'а возможно
    только на Step 4 PATH B/C, не Step 1.
  - Не запускал `1cv8.exe`.
  - Не вводил никаких реальных credentials.
  - Не делал remote push / GitHub release —
    operator action, не часть трека.
  - Не открывал Track Q Step 2 в этом же
    commit'е.
  - Не открывал параллельно ещё один трек
    (Tracks R / S / T / ... — отсутствуют как
    independent decisions; Track Q — единственный
    новый трек этого commit'а).
  - Не использовал premature closure language —
    Track Q framed как **active** (Step 1
    in-progress по уровню Track, Step 1 сам по
    себе завершён; Step 2–6 впереди); фразы
    "Windows install solved forever" /
    ...
    wizard" / "Windows service auto-magic by
    default" появляются в touched docs **только**
    как explicit DENIALS.
  - Не делал false implementation claims — Q1–Q7
    presented как **defaults** / **directional
    recommendations**, не как decided answers;
    Step 4 PATH openness preserved; technology
    choice openness preserved (Inno Setup —
    directional baseline, не lock'нутый); Q7
    framing присутствует, но decision deferred
    to Step 6.
  - Не pretend'ит, что setup.exe уже existed —
    каждое appearance "setup.exe" в touched
    docs framed как **future deliverable**
    (Step 4 PATH B target), не как
    already-shipped artefact.
  - Не уводил решение обратно в "ну wheel же
    уже есть" — план явно различает packaging
    boundary (Track M) и installer experience
    (Track Q); Track Q built **on top of**
    wheel, не over его.
- **Документ scope.** Ровно четыре файла: 2
  новых planning-документа в `docs/architecture/`
  (track-q-...-plan.md + track-q-...-step-map.md)
  + README.md (Quickstart paragraph дополнен
  Track Q active narrative alongside existing
  Track P narrative; "Active parallel track" →
  "Active parallel tracks" plural; одна новая
  Track Q секция inserted после existing
  Track P секции) + PROJECT-STATUS.md (header
  rewrite + эта section inserted после existing
  Track P / Step 1 section). Track P / Step 1
  section byte-identical; Track O closure block
  и всё previous status preserved byte-
  identical.
- **Verify-release.ps1 -AllowDirtyTree.** GREEN
  на 8 checks; никаких реальных credentials в
  Step 1 diff'е; registries `read=15 /
  write=25 / intelligence=16` без drift'а.
- **Selfcheck после Step 1.** Зелёный: registries
  `read=15 / write=25 / intelligence=16` без
  drift'а; `selfcheck_status=ok`.
- **Следующий шаг.** **Parallel Track Q / Step
  2 — baseline audit of current Windows install
  reality (docs-only).** Новый descriptive
  audit-документ
  (`docs/architecture/track-q-windows-installer-path-and-setup-exe-delivery-baseline-audit.md`)
  с inventory of ordinary-Windows-user persona
  reality; inventory of three existing install-
  adjacent paths (`scripts/release/install.ps1`,
  ...
  inventory of repository for installer-
  technology definition files (verifying none
  exist); inventory of repository for any
  bundled-runtime binaries (verifying none
  exist); option-space audit of §4 shape α
  (bundled embeddable) vs shape β (frozen exe);
  technology-choice audit Inno Setup vs WiX vs
  NSIS vs others с grounded per-option
  pro/con; expected installed footprint per
  technology; directional Q1–Q6 resolutions с
  evidence-grounded recommendations + Step 3
  handoff list (≥ 10 items). Production-код

...
> **Внимание про команды в примере выше.** Phase 5 / Step 3
> shipped honest orchestration **контракт** — он поднимает
> **те команды, которые оператор укажет**, не изобретая CLI
> за серверы. Сами `python -m mcp_read_server` /
> `python -m mcp_write_server` /
> `python -m mcp_intelligence_server` entrypoints + minimal
> stdio JSON-RPC transport + four original CLI flags
> (`--help`, `--config-path`, `--transport`, `--log-level`)
> ship'нуты позже, в Track G / Step 4 (post-Phase-6 parallel
> track). Track H / Step 4 (post-Track-G parallel track)
> расширил `--transport` value set до `{stdio,http}` и
> добавил два новых флага (`--bind <HOST>:<PORT>` +
> `--auth-token-env <VARNAME>`) для нового HTTP/1.1 `/mcp`
> endpoint'а с static bearer authentication. Step 3 runtime
> contract по-прежнему остаётся transport-agnostic —
> оператор сам решает, через какой entrypoint и какой argv
> поднимать сервис (`--transport stdio` для local subprocess
> deployment или `--transport http --bind ... --auth-token-env
> ...` для trusted-network deployment behind operator's
> reverse proxy). In-process TLS не предоставляется ни Track
> G, ни Track H.
...
- **Hostile-network transport / enterprise auth / supervisor.**
  На Phase 5 / Step 3 серверы жили как in-process модули;
  Track G / Step 4 добавил local trusted-stdio baseline (три
  `python -m <server>` entrypoint'а, stdio JSON-RPC 2.0
  transport, четыре CLI флага, `[project.scripts]` console
  entries); Track H / Step 4 добавил narrow HTTP/1.1 `/mcp`
  endpoint с static bearer authentication (case-insensitive
  scheme, constant-time token compare, fail-closed on missing/
  empty/malformed/invalid token), token sources через
  `ProductConfig.auth.tokens` (`${ENV:NAME}` env-substitution,
  literal cleartext rejected at config-load) или
  `--auth-token-env <VARNAME>` CLI flag (CLI wins, replace not
  merge), required-when-`--transport http` startup gate.
  По-прежнему **out-of-scope**: in-process TLS / HTTPS
  termination, mTLS / client certificate auth, JWT / OAuth 2.0
  / OIDC / SAML / SCIM, RBAC / ABAC / per-tool ACL / per-tenant
  isolation, token rotation endpoint / refresh tokens, session
  cookies, rate limiting, WebSocket / SSE / TCP / Unix-socket /
  named-pipe transports, supervisor daemon / systemd unit /
  Windows Service registration / hot reload / restart watcher.
  Step 3 runtime contract управляет **теми командами**, которые
  оператор явно описал (`--transport stdio` или `--transport
  http --bind ... --auth-token-env ...`), и **не** изобретает
  CLI за серверы.
...
- На Phase 5 MCP-серверы (read/write/intelligence) жили
  как in-process модули. Track G / Step 4 (post-Phase-6
  parallel track) добавил local trusted-stdio baseline
  (три `python -m <server>` entrypoint'а + stdio JSON-RPC
  transport + минимальный CLI). Track H / Step 4
  (post-Track-G parallel track) добавил narrow HTTP/1.1
  `/mcp` endpoint с static bearer authentication. По-
  прежнему out-of-scope: in-process TLS / mTLS, JWT /
  OAuth / OIDC / SAML, RBAC / multi-tenant, WebSocket /
  SSE / TCP / pipe transports, supervisor / systemd /
  Windows Service / hot reload.
...
- На момент закрытия Phase 6 у трёх MCP-серверов не было
  `__main__` / CLI / transport baseline. Этот gap закрыли
  два последовательных post-Phase-6 parallel track'а:
  Track G / Step 4 — узкий local trusted-stdio набор (три
  `python -m <server>` entrypoint'а + stdio JSON-RPC
  transport + четыре CLI флага + `[project.scripts]`
  console entries); Track H / Step 4 — narrow HTTP/1.1
  `/mcp` endpoint с static bearer authentication поверх
  того же `list_tools()` / `get_tool(name)` boundary.
  In-process TLS, mTLS / OAuth / JWT / SAML / SCIM /
  RBAC / multi-tenant, WebSocket / SSE / TCP / pipe
  transports, supervisor daemon / systemd unit / Windows
  Service registration / hot reload — по-прежнему вне
  Phase 6 и вне обоих parallel track'ов.

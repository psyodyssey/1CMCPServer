...
> stdio JSON-RPC transport + four CLI flags (`--help`,
> `--config-path`, `--transport`, `--log-level`) ship'нуты
> позже, в Track G / Step 4 (post-Phase-6 parallel track).
> Step 3 runtime contract по-прежнему остаётся transport-
> agnostic — оператор сам решает, через какой entrypoint
> и какой argv поднимать сервис; Track G ship'ит local
> trusted-stdio baseline, не network transport.
...
- **Network-grade MCP transport / auth / supervisor.** На Step 3
  серверы жили как in-process модули; Track G / Step 4 (post-
  Phase-6 parallel track) добавил local trusted-stdio
  baseline — три `python -m <server>` entrypoint'а, minimum-
  viable stdio JSON-RPC 2.0 transport, четыре CLI флага,
  `[project.scripts]` console entries. Network transport
  (HTTP / WebSocket / SSE / TCP / named pipe), authentication /
  authorization (token / mTLS / OAuth / RBAC), supervisor
  daemon / systemd unit / Windows Service registration по-
  прежнему out-of-scope; Step 3 runtime contract управляет
  **теми командами**, которые оператор явно описал, и **не**
  изобретает CLI за серверы.
...
- На Phase 5 MCP-серверы (read/write/intelligence) жили
  ...
  три `python -m <server>` entrypoint'а + minimum-viable
  stdio JSON-RPC transport + минимальный CLI; network
  transport / auth / supervisor по-прежнему out-of-scope.
...
- На момент закрытия Phase 6 у трёх MCP-серверов не было
  `__main__` / CLI / transport baseline. Этот gap закрыл
  parallel Track G / Step 4 (post-Phase-6) узким local
  trusted-stdio набором — три `python -m <server>`
  entrypoint'а + minimum-viable stdio JSON-RPC transport +
  четыре CLI флага + `[project.scripts]` console entries.
  Network transport, authentication / authorization,
  supervisor daemon / systemd / Windows Service —
  по-прежнему вне Phase 6 и вне Track G.
